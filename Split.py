...
class ClientWiseNoisySplitter:
    ...
    def __repr__(self):
        return f'{self.__class__.__name__}(client_num={self.client_num}, Noisy Client ID={self.noisy_client_ids})'
    ...
    def __call__(self, dataset):
        ...
        if isinstance(dataset, ImbalancedNoisyDataWrapper):
            if dataset.corruption_prob == 0:
                ...
                for i, idx in enumerate(idxs_slice):
                    self.clients_dataidx_map[i] = idx
                fipped_idxs_slice = np.array_split(noisy_idxs, len(self.noisy_client_ids))
                for i, client_id in enumerate(self.noisy_client_ids):
                    original_idx = self.clients_dataidx_map[client_id]
                    noisy_idx = fipped_idxs_slice[i]
                    print(f'Client {client_id} has {len(noisy_idx)} noisy samples')
                    self.clients_dataidx_map[client_id] = np.append(original_idx, noisy_idx, axis=0)
                print('Splitting dataset into {} clients.'.format(self.client_num))
                print({id: len(idxs) for id, idxs in self.clients_dataidx_map.items()})
...
class ClientWiseImbalanceSplitter:
    def __init__(self, client_num, imbalanced_client_ids, imbalanced_ratio=3):
        self.client_num = client_num 
        self.imbalanced_client_ids = imbalanced_client_ids
        if len(self.imbalanced_client_ids) > self.client_num or max(self.imbalanced_client_ids) > self.client_num-1:
            raise ValueError('Noisy client ids should be less than client num.') 
        self.clients_dataidx_map = dict.fromkeys(list(range(client_num)))
        self.imbalanced_ratio = imbalanced_ratio
    def __repr__(self):
        return f'{self.__class__.__name__}(client_num={self.client_num}, Imbalanced Client ID={self.imbalanced_client_ids})'
    def __call__(self, dataset):
        if not isinstance(dataset, ImbalancedNoisyDataWrapper):
            ...
            for i, idx in enumerate(idxs_slice):
                self.clients_dataidx_map[i] = idx
            for imbalanced_client in self.imbalanced_client_ids:
                idx = self.clients_dataidx_map[imbalanced_client]
                ...
                self.clients_dataidx_map[imbalanced_client] = selected_idx
                print(f'Client {imbalanced_client} portion per class: {portion_per_class}')
            print('Splitting dataset into {} clients.'.format(self.client_num))
...
if __name__ == "__main__":
    ...
    flipped_imnbalanced_dataset = ImbalancedNoisyDataWrapper(
            ...
            imbalanced_ratio=3,
            num_classes=10)
    client_noise_splitter = ClientWiseNoisySplitter(client_num, noisy_client_ids=[0, 1])
    clients_dataidx_map = client_noise_splitter(flipped_imnbalanced_dataset)
    client_imbalanced_splitter = ClientWiseImbalanceSplitter(client_num, imbalanced_client_ids=[0, 1])
